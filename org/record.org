...
* txqueuelen
Here we can see how to increase the txqueuelen for a Ethernet port in Linux

txqueuelen = Transfer Queue Length

We may ask the Question What’s the need of Increasing this ?

The need of increasing this value to higher will make the transfer queue more length
If we have a server machine we need to assign this value to high, because the transfer rate
want to be hight to server the data’s for all client machine’s.

For Desktop PC’s we Don’t need to Assign this value
By Default value of the txq is 1000, if we need to increase it Use the command
#+begin_example
# ifconfig eth0 txqueuelen 10000
#+end_example
This will make the changed from 1000 to 10000

If we need to make the value for permanent, Add the Command to rc.local file to get run while boot

Edit the file
#+begin_example
# vim /etc/rc.local 
#+end_example
Enter the command as follow for Respective Ethernet Port’s
#+begin_example
# ifconfig eth0 txqueuelen 10000
